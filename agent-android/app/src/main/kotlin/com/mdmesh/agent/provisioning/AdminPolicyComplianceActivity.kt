 ...
 * — which carries our single-use enroll token — is delivered to THIS activity's intent,
 * not to [AdminReceiver.onProfileProvisioningComplete]. So this is where we capture the
 * token, persist it, and kick an immediate check-in so the device enrolls within seconds.
 */
class AdminPolicyComplianceActivity : Activity() {
    ...
    private fun extrasToken(): String? =
        intent.getParcelableExtra<PersistableBundle>(
            DevicePolicyManager.EXTRA_PROVISIONING_ADMIN_EXTRAS_BUNDLE,
        )?.getString(AdminReceiver.EXTRA_ENROLL_TOKEN)
}
